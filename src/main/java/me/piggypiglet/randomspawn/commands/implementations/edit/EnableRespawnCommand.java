...
// ------------------------------
public final class EnableRespawnCommand extends Command {
    @Inject private SpawnManager spawnManager;
    @Inject private Data data;

    public EnableRespawnCommand() {
        super("enablerespawn", "Enable respawning on a spawn.", "<location name>", "randomspawn.admin", "randomspawn.respawn.enable");
    }
    ...
    protected boolean execute(CommandSender sender, String[] args) {
        if (args.length > 0) {
            ...
            if (opSpawn.isPresent()) {
                ...
                spawn.setRespawn(true);

                data.editSpawn(spawn);
                spawnManager.getSpawns().add(spawn);
                spawnManager.getRespawnable().add(spawn);

                sender.sendMessage(getMessage(ENABLERESPAWN_SUCCESS, args[0]));
            } else {
