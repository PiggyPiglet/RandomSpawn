...
public final class ConfigMapper implements ObjectMapper<MutableFileConfiguration, Config> {
    ...
    public Config dataToType(MutableFileConfiguration config) {
        return new Config(
                OPTIONS_MAPPER.dataToType(((AbstractFileConfiguration) config.getConfigSection("options")).getAll()),
                SPAWNS_MAPPER.dataToType((Map<String, Map<String, Object>>) config.get("data"))
        );
    }
